...
Canonical subagent definitions live in `.codex/agents/*.toml`. When alternate-runtime projections are present, regenerate them with the repository's synchronizers after changing a canonical TOML and use their `--check` mode to detect stale, missing, or unmapped outputs.
...
The parent prompt must require the `MESHY_API_KEY` hard gate and autonomous 3D bootstrap before path discovery or provider work; an Internet search across eligible game, tabletop, fantasy/horror, and professional design artwork when no authoritative source exists; exclusion of archival, museum, historical, antiquities, archaeological, ethnographic, reenactment, and documentary material from the model-reference pool; immutable source and search evidence; faithful ImageGen cleanup with a visual-fidelity comparison; and explicit parent or user direction before a from-scratch fallback. It must also require Meshy 7 (`meshy-7`) as the default generation model, exactly one approved Meshy input image, no multi-view provider board, immediate provider download and checksum, protected provider source, topology repair, PDX packed-material validation, hash-aware runtime synchronization, and `.mesh`/`.anim` reimport evidence. Normal generation, planned model/animation operations, and failure-driven provider recovery are pre-authorized while live balance and verified provider capability permit them; stop only for insufficient credits, provider refusal, or an unavailable capability, and record every attempt. For required skeletal motion, require substantive actions from verified `meshy_animate` or an explicitly user-approved professional source; Blender may import, retarget, clean, correct contacts or roots, bake, validate, and export, but may not author final replacement motion. For custom units, require Internet sound-source research, immutable original downloads, source URLs, licensing and usage evidence, access dates, checksums, animation synchronization points, and a blocked state when no defensible sourced file exists; generated, synthesized, recorded, manually authored, placeholder, test-tone, and unlicensed audio is forbidden. Selection audio is mandatory: require its stable identifier, exact engine consumer and binding scope, effective country or original tag, every consumer sharing that identity, and actual selection evidence rather than idle or entity-state evidence. If the engine cannot bind distinct per-subunit selection voices under a shared tag, record that limitation as a blocker. Also require bespoke counters for every used counter surface, mandatory inspection of the exact installed-vanilla counter definition/DDS and matching reference family, and the sampled vanilla green palette; reused counters, arbitrary green, and unreferenced imitations are forbidden. For humanoids, name the installed vanilla source mesh and entity and pass the source-height/entity-scale/effective-runtime crosswalk; for buildings, pass the valid state/province pair and entity visibility test.
...
For probability work, `hoi4_ai_probability_auditor` must start with `hoi4.probability_inspect`, name the analyzed surface and scenario ids, preserve the scenario hash, state whether the candidate pool and external factors were complete, and distinguish exact, bounded, sampled, score-only, and unresolved results. It must use `hoi4.probability_evaluate`, `hoi4.probability_sweep`, and `hoi4.probability_compare` according to the scenario, with simulation, sequence analysis, and rendering only under their declared evidence conditions. For technology or doctrine work, list the affected technology, folder, unlock, grant, bonus, or asset IDs and require the installed read-only `hoi4.tech_inspect`, `hoi4.tech_render`, and `hoi4.tech_compare` routes. If an exact route is unavailable, report the exact missing tool and block that evidence instead of inventing viewer evidence; source-only review is not equivalent.
