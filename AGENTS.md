...
This file describes how you should read, edit and extend the `Pax Franca` codebase.

## 0. Required Reading Before Any Change

### Paradox Wiki

Before you open or edit any `Pax Franca` file, you must consult the relevant Hearts of Iron IV modding pages from the offline Paradox wiki snapshot in `paradox_wiki/`.
...
If `Pax Franca` already has a pattern for the same thing, follow that over vanilla for consistency, but still take a look at a vanilla implementation.
...
13. Prefer reusable dynamic scripted effects/triggers for complex/dynamic logic.
    - First check existing dynamic effects (in `common/scripted_effects/pf_dynamic_effects.txt`) and use them instead of duplicating logic.
    - If no existing effect fits, create a new dynamic effect and document it in the matching markdown file `docs/pf_dynamic_effects.md` in the same change.
...
Add prefixes if a folder is dedicated to `Pax Franca` files that all share the prefix. Do not add the `pf` prefix to variable names, scripted effects, or scripted triggers unless the surrounding context uses it consistently.
...
8. Reuse existing dynamic scripted effects before writing new bespoke logic. If new dynamic effects/triggers are added, document them in `docs/pf_dynamic_effects.md` in the same change.
...
13. Respect the repository style and naming rules so new content blends with existing `Pax Franca` code.
